//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by Entitas.CodeGeneration.Plugins.EventSystemsGenerator.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
public sealed class UIEventSystems : Feature {

    public UIEventSystems(Contexts contexts) {
        Add(new UIActiveEventSystem(contexts)); // priority: 0
        Add(new UIActiveRemovedEventSystem(contexts)); // priority: 0
        Add(new UIPositionEventSystem(contexts)); // priority: 0
    }
}
